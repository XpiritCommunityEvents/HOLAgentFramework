...
string prompt =
    ...
    Provide information about the concerts in the chat and add location details so I can set pins on this map with a LLM"
    """;
...
ImageClient imageClient = new AzureOpenAIClient(new Uri(endpoint), new ApiKeyCredential(apiKey))
...
// Extract all text parts into a single string
string promptText = $"Here is the image of the San Diego area: https://github.com/renevanosnabrugge/llm-images/blob/main/SanDiego-Area.png. draw pins on this image based on this information {result}";
//string promptText = $"Draw pins of the San Diego Area based on this information {result}";

ClientResult<GeneratedImage> image = await imageClient.GenerateImageAsync(promptText, options);
Console.WriteLine("Image URL: " + image.Value.ImageUri);
